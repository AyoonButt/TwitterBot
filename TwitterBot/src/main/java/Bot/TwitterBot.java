...
public class TwitterBot {
    ...
    private void initiateOAuth2() {
        oauth20Service = new ServiceBuilder("your_client_id")
                ...
                .callback("http://localhost:4567/authorize")
...
public class TwitterBot {
    ...
    public TwitterBot(String bearerToken) {
    ...
    public static void main(String[] args) {
        ...
        TwitterBot twitterBot = new TwitterBot(bearerToken);
        twitterBot.tweetQuotes();
    }
    ...
    private void authenticate() {
        apiInstanceJava = new TwitterApi(new TwitterCredentialsBearer(bearerToken));
    }
    ...
    private void tweetQuotes() {
        for (String tweet : tweets) {
            ...
            sleepForInterval(1800000); // Sleep for 30 minutes
        }
    }
    ...
    private void createAndSendTweet(String tweetText) throws ApiException {
        TweetCreateRequest tweetCreateRequest = new TweetCreateRequest();
        tweetCreateRequest.setText(tweetText);
        apiInstanceJava.tweets().createTweet(tweetCreateRequest).execute();
    }
...
import com.twitter.clientlib.TwitterApi;
import com.twitter.clientlib.TwitterCredentialsOAuth2;
import com.twitter.clientlib.api.TwitterApi;
import com.twitter.clientlib.model.TweetCreateRequest;
import com.twitter.clientlib.ApiException;
import com.twitter.clientlib.ApiClientCallback;
import com.twitter.clientlib.model.OAuth2AccessToken;
...
public class TwitterBot {
    ...
    public TwitterBot(String apiKey, String apiSecretKey, String accessToken, String refreshToken) {
        this.credentials = new TwitterCredentials(apiKey, apiSecretKey, accessToken, refreshToken);
        authenticate();
        tweets = new ArrayList<>();
        loadTweetsFromFile("C:\\Users\\ayoon\\projects\\TwitterBot-1\\TwitterBot\\src\\main\\resources\\tweets.txt");
    }
    ...
    public static void main(String[] args) {
        // Use your actual API key, API secret key, access token, and refresh token
        String apiKey = "YOUR_API_KEY";
        String apiSecretKey = "YOUR_API_SECRET_KEY";
        String accessToken = "YOUR_ACCESS_TOKEN";
        String refreshToken = "YOUR_REFRESH_TOKEN";

        TwitterBot twitterBot = new TwitterBot(apiKey, apiSecretKey, accessToken, refreshToken);
        twitterBot.tweetQuotes();
    }
    ...
    private void authenticate() {
        apiInstanceJava = new TwitterApi(new TwitterCredentialsOAuth2(credentials));
        apiInstanceJava.addCallback(new MaintainToken());
        startTokenRefreshThread();
    }

    private void startTokenRefreshThread() {
        // Schedule token refresh in a background thread
        Thread refreshTokenThread = new Thread(() -> {
            while (true) {
                try {
                    // Refresh token every hour (adjust as needed)
                    Thread.sleep(60 * 60 * 1000);
                    apiInstanceJava.refreshToken();
    ...
    static class MaintainToken implements ApiClientCallback {
        @Override
